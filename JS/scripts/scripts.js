	...
	*/
	...
	function openContent(p_id)
	{
		...
		//else{
			...
			newPosX= $("#" + currentContent + "_content").offset().left - menuWidth;//$("#sommaire").width();
	...
	function openMenu(p_select, p_titre)
	{
		...
		if(myResolutionMode==2 && lastMenu!=undefined && lastMenu!=$(p_select)) // Si la r�solution est petite
		{
	...
	function onResize()
	{
		...
		$(".content").css("width", wWidth - menuWidth);
		...
		{
			// Changement de mode de r�solution
			myResolutionMode=2;
			alert("petit");
		...
		if((wWidth>minWidth) && myResolutionMode==2)
		{
			...
			$(".content").css({"top":"0px", "padding":"30px", marginLeft:"280px", fontSize:"18pt"});
...
alert(isMobile.Android());*/
	...
	/*document.querySelectorAll("code").forEach(function(element, num) {
