...
tabSum.push(new Array("exo", "Exercice - Tableaux", "exercice_tableaux.html", "dev" ));
...
tabSum.push(new Array("link", "Reset", "reset.html", "dev"));
tabSum.push(new Array("link", "Syntaxe", "syntaxe_css.html"));
...
tabSum.push(new Array("link", "Filtres et effets", "filtres.html", "dev,graph"));
tabSum.push(new Array("link", "Transformations", "transform.html", "dev,graph"));
tabSum.push(new Array("link", "Transitions", "transition.html", "dev,graph"));
tabSum.push(new Array("link", "Animations", "animations.html", "dev,graph"));

tabSum.push(new Array("link", "Media queries", "media_queries.html", "dev,graph"));
...
function createSum()
{
	...
	{
		let deniedClass = "";
		if(tabSum[i][3] && tabSum[i][3].indexOf(profile) == -1)
		{
			deniedClass = "denied";
		}
		...
		if(tabSum[i][0] == "link")
		{
			...
			let newButtonHTML = '';
			newButtonHTML += '<button class="sumItem ' + deniedClass + '" onclick="openPage('+i+');" >';
			newButtonHTML += 	'<span class="numSum">' + textNum + '</span>';
			newButtonHTML += 	'<span id="span_' + i + '" class="aSum ' + (tabSum[i][3] || '' ) + ' ' + deniedClass + '">'+tabSum[i][1]+'</span>';
			newButtonHTML += '</button>';

			sum.innerHTML += newButtonHTML;
		}
		if(tabSum[i][0] == "title")
		{
			idxTitle++;
		...
		if(tabSum[i][0] == "exo")
		{
			sum.innerHTML += '<button class="sumItem ' + deniedClass + '" onclick="openPage('+i+');"><span class="exoSum ' + (tabSum[i][3] || '' ) + '">'+tabSum[i][1]+'</span></button>';
		}
		if(tabSum[i][0] == "url")
		{
			sum.innerHTML += '<div class="sumItem"><a class="exoSum ' + (tabSum[i][3] || '' ) + ' ' + deniedClass  + '" href="' + tabSum[i][2] + '" target="_blank">' + tabSum[i][1] + '</a></div>';
		}
	}
	//	document.write('</div>');
	sum.innerHTML += '</nav>';
...
function displayProfileSpec()
{
	document.querySelectorAll(".profile_hidden").forEach((el)=>
	{
